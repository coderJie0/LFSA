...
from utils import gradcam_and_crop, get_feature
from distances import LpDistance
import other_attacks
import random
from torchvision.transforms import ToPILImage
import torchvision
def deprocess(tensor, res=512):
    # 1. 将 [-1, 1] 归一化回 [0, 1]
    tensor = (tensor + 1.0) / 2.0

    # 2. 将 PyTorch 张量转换为 NumPy 数组
    tensor = tensor.squeeze().cpu().numpy()  # 移除批处理维度并将其转回 NumPy 数组

    # 3. 将通道维度移到最后
    tensor = tensor.transpose(1, 2, 0)

    # 4. 将图像值从 [0, 1] 映射回 [0, 255]
    tensor = (tensor * 255.0).clip(0, 255).astype(np.uint8)

    # 5. 将 NumPy 数组转换回 PIL 图像
...
def ddim_reverse_sample(image, prompt, model, num_inference_steps: int = 20, guidance_scale: float = 2.5,
    ...
    # print(model.scheduler.timesteps)
    # tensor([951, 901, 851, 801, 751, 701, 651, 601, 551, 501, 451, 401, 351, 301, 251, 201, 151, 101,  51,   1])

    latents = encoder(image, model, res=res)
    timesteps = model.scheduler.timesteps.flip(0) # 顺序反转
    ...
    return latents, all_latents # 0、1、2、3、...19
...
def diffattack(
    ...
    pred = classifier(test_image.to(device))
    ...
    """
    # 1
    aug_image = STM(test_image, gamma=0.01)
    # print(type(image))
    latent, inversion_latents = ddim_reverse_sample(image, prompt, model,
                                                    num_inference_steps,
                                                    0, res=height)
    aug_latent, aug_inversion_latents = ddim_reverse_sample(aug_image, 
                                                    prompt, model,
                                                    num_inference_steps,
    ...
    latent = inversion_latents[start_step - 1]# 5
    aug_latent = aug_inversion_latents[start_step - 1]
    ...
    aug_latent, aug_latents = init_latent(aug_latent, model, height, width, batch_size)
    ...
    for ind, t in enumerate(tqdm(model.scheduler.timesteps[1 + start_step - 1:], desc="Optimize_uncond_embed")):
        for _ in range(10 + 2 * ind):
            ...
            aug_out_latents = diffusion_step(model, aug_latents, context, t, guidance_scale)
            optimizer.zero_grad()
            # loss = loss_func(out_latents, inversion_latents[start_step - 1 + ind + 1])
            loss = loss_func(out_latents, inversion_latents[start_step - 1 + ind + 1]) + loss_func(aug_out_latents, aug_inversion_latents[start_step - 1 + ind + 1])
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            aug_latents = diffusion_step(model, aug_latents, context, t, guidance_scale)
            p = random.random()
    ...
    aug_original_latent = aug_latent.clone()

    latent.requires_grad_(True)
    aug_latent.requires_grad_(True)
    ...
    aug_optimizer = optim.AdamW([aug_latent], lr=1e-2)
    ...
    for iter, _ in enumerate(pbar):
        controller.loss = 0

        #  The DDIM should begin from 1, as the inversion cannot access X_T but only X_{T-1}
        controller.reset()
        latents = torch.cat([original_latent, latent])
        aug_latents = torch.cat([aug_original_latent, aug_latent])
        ...
        #         print(f"perturbations[{i}] updated: {torch.norm(perturb.grad)}")
        #     else:
        #         print(f"perturbations[{i}] not updated")
        ...
        if verbose:
            pbar.set_postfix_str(
                ...
                f"loss: {loss.item():.5f} "
                f"pred: {torch.argmax(pred, 1).detach().item()}"
                )
