...
# parser.add_argument('--images_root', default="dataset/Stanford_Car/images", type=str,  help='The clean images root directory')
# parser.add_argument('--images_root', default="result/Stanford-Car/carResnet50-sty_p=0.2-beta=0.1-pertur_weight=0.15-self*t", type=str,  help='The clean images root directory')

parser.add_argument('--label_path', default="dataset/imagenet-compatible/labels.txt", type=str,
# parser.add_argument('--label_path', default="dataset/CUB_200_2011/labels.txt", type=str,
# parser.add_argument('--label_path', default="dataset/Stanford_Car/labels.txt", type=str,
...
parser.add_argument('--pretrained_diffusion_path', default="stabilityai/stable-diffusion-2-base", type=str, help='Change the path to `stabilityai/stable-diffusion-2-base` if want to use the pretrained model')
...
# parser.add_argument('--dataset_name', default="cub_200_2011", type=str, choices=["imagenet_compatible", "cub_200_2011", "standford_car"], help='The dataset name for generating adversarial examples')
# parser.add_argument('--dataset_name', default="standford_car", type=str, choices=["imagenet_compatible", "cub_200_2011", "standford_car"], help='The dataset name for generating adversarial examples')
...
parser.add_argument('--attack_loss_weight', default=1, type=int, help='attack loss weight factor')# 1
...
if __name__ == "__main__":
    ...
    #image_id_list,label,label_tar_list = load_ground_truth('./dataset/images.csv')
    #label = np.array(label)
    ...
    if is_test:
        path=save_dir
        all_clean_images = glob.glob(os.path.join(path, "*originImage*"))
        all_clean_images = natsorted(all_clean_images, alg=ns.PATH)
        all_adv_images = glob.glob(os.path.join(path, "*adv_image*"))
